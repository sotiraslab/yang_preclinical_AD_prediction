...
if (INTERACTIVE) {
    ...
    opt$odir <- file.path(opt$wdir, "code/exploratory/a4_treatment/pet_analysis")
    opt$reload <- FALSE
}
...
a4_pet <- read_csv("/home/b.y.yang/sotiraslab/BradenADLongitudinalPrediction/code/exploratory/a4_treatment/a4_xs.csv")
features_old <- read_csv("/home/b.y.yang/sotiraslab/BradenADLongitudinalPrediction/data/features/features.csv")
...
pet_analysis_wdir <- "/ceph/chpc/shared/aristeidis_sotiras_group/b.y.yang_scratch/pet_analysis_A4_treatment"
...
muse_wdir <- "/ceph/chpc/shared/aristeidis_sotiras_group/b.y.yang_scratch/T1_MUSE_A4_treatment"
...
qc_df <- read_csv("code/exploratory/a4_treatment/pet_analysis/qc.csv", show_col_types = FALSE) %>%
    rename(idvi = id)
...
a4_pet_filter <- a4_pet %>%
    select(a4_substudy:filepath.from_tabular.mri, contains("raw_t1"), site:long_pacc, -summary_cortical_amyloid) %>%
    mutate(
...
# newly processed scans
a4_pet_new <- a4_pet_filter %>%
    filter(idvi %in% suvr_df_select$idvi) %>%
    left_join(  # merge SUVR
    ...
    left_join(qc_df, by="idvi") %>%
    mutate(
        site_in_pac = FALSE
    )

# old scans
a4_pet_old <- features_old %>%
    filter(
        idvi %in% (a4_pet_filter %>% filter(!(idvi %in% suvr_df_select$idvi)) %>% pull(idvi))
    )
...
# merge with old processed data
features_final <- bind_rows(features_df_filter, a4_pet_old) %>%
    left_join(a4_pet %>% select(idvi, TX), by = "idvi")
...
if (SAVE_CSV) {
    ...
    write_csv(features_final, file.path(opt$odir, "features_A4_treatment.csv"))
